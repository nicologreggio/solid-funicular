...
{
    $searchbarStr = "
        <label for='searchbar'>Cerca per nome:</label><br />
...
{
    $categoriesStr = "
        <label for='categories'>Cerca per categoria:</label><br />
...
function fillMaterials($page)
{
    ...
    {
        $materialsStr .= "<span class='search-material'><input id='search-material-{$index}' type='checkbox' name='materials[]' value='{$material->getId()}' ";
        ...
        $materialsStr .= "/><label for='search-material-{$index}'>{$material->getName()}</label></span>";
...
echo fillPage(file_get_contents("./search/search.html"));
